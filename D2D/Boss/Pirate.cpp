	...
	: captainInfo(captainInfo), boatInfo(boatInfo), maxHp(maxHp), waveValue(waveValue)
{
	...
	minMaxWave = Vector2(boatInfo.position.y - waveValue, boatInfo.position.y + waveValue);
...
//	cout << ((int)((float)hp / (float)maxHp * 100)) << '\n';
	...
	if (direction)	// 상
	{
		...
		if (boatInfo.position.y >= minMaxWave.y)
			direction = false;
	}
	else	// 하
	{
		...
		if (boatInfo.position.y <= minMaxWave.x)
			direction = true;
	}
...
void Pirate::GUI()
{
	...
	if (ImGui::Begin("Pirate", &bOpen))
	{
		...
		{
			boatInfo.position.y = minMaxWave.x + waveValue;
			waveValue = tempWave;
			minMaxWave = Vector2(boatInfo.position.y - waveValue, boatInfo.position.y + waveValue);
		}
		...
		text = "Boat";
		ImGui::Text(text.c_str());
		if (ImGui::SliderFloat2("Position", (float*)&boatInfo.position, CAMERA->GetPosition().x, CAMERA->GetPosition().x + WIN_DEFAULT_WIDTH))
		{
			direction = true;
			speed = 0.4f;
			minMaxWave = Vector2(boatInfo.position.y - waveValue, boatInfo.position.y + waveValue);
